...
function frequency(y::Array{Float64,1})
  ...
  for i=1:length(y)-1
    if spk==0 && y[i]<-1. && y[i+1]>=-1.
      spk += 1
      N0 = copy(i)
    elseif spk==1 && y[i]<-1. && y[i+1]>=-1.
      N1 = copy(i)
      break
    end
  end
...
function check_rfp(v0::Real, w0::Real, I_app::Real, epsilon::Real)
  ...
  jacobian(fps_v::Real, fps_w::Real) = [ [1 - fps_v^2, -2 * fps_w]; [(epsilon*50*exp(5*v0 - 5*fps_v + 5/3))/(exp(5*v0 - 5*fps_v + 5/3)+ 1)^2, -epsilon] ]
...
function ficurve(v0::Real, w0::Real, I_app_range::Vector{Float64}, epsilon::Real, v_init::Real, w_init::Real, t::Array{Float64,1})
  f = zeros(length(I_app_range))
  for i=1:length(I_app_range)
    println(I_app_range[i])
    stable_rfp = check_rfp(v0, w0, I_app_range[i], epsilon)
    if stable_rfp == 0
      (v, w) = FM_forward_euler_winf_new2(v0, w0, I_app_range[i], epsilon, v_init, w_init, t)
      global v = v
      f[i] = frequency(v[t500:end])
    elseif stable_rfp == 1
      f[i] = NaN
    end
  end
